    ...
    final found =
        widget.stops
            .map(
              (s) => (
                s,
                getDistance(center.latitude, center.longitude, s.lat, s.lng),
              ),
            )
            .where((e) => e.$2 <= 1.0)
            .toList()
  ...
  Widget build(BuildContext context) {
    return Consumer<AppState>(
      builder: (context, state, _) {
        final markers = _nearby.map((e) {
          final (displayName, subtitle) = getDisambiguatedStopDisplay(
            e.$1,
            state.stops,
          );
          return dotMarker(
            LatLng(e.$1.lat, e.$1.lng),
                  ...
                  child: Column(
                    ...
                    children: [
                      Padding(
                        ...
                        child: Text(
                          'အနီးဆုံးမှတ်တိုင်များ (${_nearby.length})',
